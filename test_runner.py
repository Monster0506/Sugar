...
class TestResult:
    ...
    def __init__(self, filename: str, success: bool, runtime: float, 
...
class SugarTestRunner:
    ...
    def __init__(self, interpreter_file: str, timeout: int = 30, verbose: bool = False, 
                 quiet: bool = False, stop_on_error: bool = False):
    ...
    def run_interpreter_on_file(self, sugar_file: str) -> TestResult:
    ...
    def run_tests(self, files: List[str]) -> None:
        ...
        for i, sugar_file in enumerate(files, 1):
            if not self.quiet:
                print(f"[{i:3d}/{len(files)}] Testing {sugar_file}... ", end="", flush=True)
...
Examples:
  ...
  uv run test_runner.py interpreter.py --all --timeout 60
    ...
    parser.add_argument(
        "-f", "--file",
        help="Run interpreter against a specific .sugar file"
    )
