...
class ErrorReporter:
    ...
    def report_semantic(self, message: str, node: Node):
        ...
        if not node.meta:
            print(f"Semantic Error in {self.file_path}: {message} (no location info)", file=sys.stderr)
            return
    ...
    def report_runtime(self, message: str, node: Node | None):
        ...
        if not node or not node.meta:
            print(f"Runtime Error in {self.file_path}: {message} (no location info)", file=sys.stderr)
            return
